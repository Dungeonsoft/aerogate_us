...
public sealed class FB : ScriptableObject
{
    ...
    {
        FacebookImpl.AppRequest(message, actionType, objectId, to, null, null, null, data, title, callback); //1Àº ¿ø·¡ null//
    }
    ...
    public abstract class RemoteFacebookLoader : MonoBehaviour
    {
        ...
        public static IEnumerator LoadFacebookClass(string className, LoadedDllCallback callback)
        {
            ...
            if (www.error != null)
            {
                ...
                GameObject.Find("FacebookLogin").GetComponent<FaceBookUseScript>().facebookBTN.SetActive(true);
